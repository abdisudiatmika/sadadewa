...
// DELETE /api/fees/:id/unpaid-bills - Delete all unpaid bills generated by this template
router.delete("/:id/unpaid-bills", async (req: Request, res: Response) => {
  try {
    const result = await feeService.deleteUnpaidBills(req.params.id as string);
    res.json({ success: true, ...result });
  } catch (error: any) {
    res.status(400).json({ success: false, error: error.message });
  }
