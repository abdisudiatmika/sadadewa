...
export default function BillingHistoryPage() {
  ...
  const fetchBilling = async () => {
    ...
    try {
      const res = await api.getStudentBilling(studentId);
  ...
  const tabs = [
    { id: 'tunggakan', label: 'Tunggakan', icon: 'warning' },
    { id: 'riwayat', label: 'Riwayat Pembayaran', icon: 'history' },
  ];

  const studentName = transactions[0]?.student?.fullName;
  ...
  return (
    <div className="flex-1 flex flex-col gap-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant mt-1 m-0">
            {studentId 
              ? `Menampilkan data tagihan dan histori transaksi untuk ${studentName || 'siswa terpilih'}.`
      ...
      {/* Tabs - only when studentId is present */}
      {studentId && (
        <div className="flex gap-2">
          {tabs.map(tab => (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-label-lg transition-all ${
                activeTab === tab.id
      ...
      {/* Tunggakan Tab Content */}
      {studentId && activeTab === 'tunggakan' && (
        <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden shadow-sm">
          <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
            <table className="w-full border-collapse">
              <thead className="sticky top-0 z-10 bg-surface-container shadow-sm">
                <tr className="bg-surface-container">
                  <th className="text-left p-4 font-label-lg text-on-surface-variant border-b border-outline-variant">Jenis Tagihan</th>
                  ...
                  <>
                    {outstandingItems.map((item) => (
                      <tr key={item.id} className="hover:bg-surface-container-low transition-colors">
                        <td className="p-4 border-b border-outline-variant font-body-md text-on-surface font-medium">
                          {item.feeTemplate?.name || 'Item'}
                        </td>
                        <td className="p-4 border-b border-outline-variant font-body-md text-on-surface">
                          {item.billingMonth ? `${monthNames[item.billingMonth]} ${item.billingYear}` : item.billingYear || '-'}
                        </td>
                        ...
                        <td className="p-4 border-b border-outline-variant text-center">
                          <span className={`px-2.5 py-1 rounded-md font-label-md capitalize ${getStatusBadge(item.status)}`}>
                            {item.status}
                          </span>
                        </td>
                      </tr>
                    ...
                    <tr className="bg-surface-container font-bold">
                      <td colSpan="2" className="p-4 text-right font-label-lg text-on-surface">
                        Total Tunggakan
                      </td>
      ...
      {/* Riwayat Pembayaran Tab Content (or default when no studentId) */}
      {(!studentId || activeTab === 'riwayat') && (
        <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden shadow-sm">
          {/* Toolbar */}
